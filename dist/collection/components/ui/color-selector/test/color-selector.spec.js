...
describe('color-selector', () => {
    ...
    it('should render color-selector with colors', async () => {
        const page = await newSpecPage({
            components: [ColorSelector],
            template: () => h("color-selector", { colors: colors }),
        });
        expect(page.root).toEqualHtml(`
      <color-selector>
    ...
    it('should render color-selector with selected color', async () => {
        const page = await newSpecPage({
            components: [ColorSelector],
            template: () => h("color-selector", { colors: colors, selectedId: 2 }),
        });
        expect(page.root).toEqualHtml(`
      <color-selector>
        <div class="color-selector">
    ...
    it('should emit colorSelected event when color is clicked', async () => {
        ...
        const colorButton = page.root.querySelector('.color-option');
        colorButton.click();
        expect(colorSelectedSpy).toHaveBeenCalledWith(expect.objectContaining({
            detail: colors[0],
        }));
    });
    it('should handle colors with image background', async () => {
        const page = await newSpecPage({
            components: [ColorSelector],
            template: () => h("color-selector", { colors: colorsWithImage }),
        });
        const colorDot = page.root.querySelector('.color-dot');
...
//# sourceMappingURL=color-selector.spec.js.map
